...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BHN address (e.g. %1)").arg("MMcJVNK6LNTKBpZ5hscqciLjwBbxkCknzu"));
#endif
...
{
    // return if URI is not valid or is no BHN: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert bhn:// to bhn:
    //
    //    Cannot handle this later, because bhn:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bhn.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BHN.lnk";
}
...
{
    return GetAutostartDir() / "bhn.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bhn.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bhn app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bhn app to startup item list
